...
namespace MajdataPlay.Scenes.Game
{
    public interface INoteTimeProvider
    {
        ...
        Dictionary<string, List<Tuple<float, float>>> SVLists { get; } //time, sveloc
        Dictionary<string, List<Func<float, float>>> PositionFunctionses { get; }

        //计算SV函数
        public void CalcSVPos()
        {
            foreach (var d in SVLists)
            {
                var SVList = d.Value;
                var PositionFunctions = PositionFunctionses[d.Key] = new();
                ...
                if (SVList.Count == 1)
                {
                    ...
                    MajDebug.LogInfo($"[SV {d.Key}] Single Segment Case: Start = {lastPosition}, Speed = {SVList[0].Item2}");
                    continue;
                }
                for (int i = 0; i < SVList.Count - 1; i++)
                {
                    ...
                    MajDebug.LogInfo($"[SV {d.Key}] Segment Case {i}: startTime = {lastTime}, Start = {lastPosition}, Speed = {lastSpeed}");
                ...
                MajDebug.LogInfo($"[SV {d.Key}] Segment Case Last: StartTime = {lastTime}, Start = {lastPosition}, Speed = {lastSpeed}");
            }
        }
        public float GetPositionAtTime(float AudioT, string subSV = "1")
        {
            try
            {
                var SVList = SVLists[subSV];
                var PositionFunctions = PositionFunctionses[subSV];
